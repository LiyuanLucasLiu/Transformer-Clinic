...
class FairseqTask(object):
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False):
        ...
        if self.args.plot_stability:
            if self.args.gradient_as_delta:
                loss, sample_size, logging_output = criterion(model, sample)
                optimizer.backward(loss)
                for name, parameter in model.named_parameters():
                    if 'bias' not in name and 'layer_norm' not in name and 'ratio' not in name and parameter.requires_grad:
                        parameter.data.add_(parameter.grad.data.sign(), alpha=-0.000001)
                loss, sample_size, logging_output = criterion(model, sample)
            else:
                with torch.no_grad():
                    loss, sample_size, logging_output = criterion(model, sample)
                    for name, parameter in model.named_parameters():
                        if 'bias' not in name and 'layer_norm' not in name and 'ratio' not in name:
                            parameter.data.add_(parameter.data.clone().normal_().sign(), alpha=-0.0001)
                    loss, sample_size, logging_output = criterion(model, sample)
            exit()
        model.train()
        loss, sample_size, logging_output = criterion(model, sample)
